...
**3. Backend had no CORS configuration — app looked completely broken in a real browser**
- What it produced: the frontend rendered nothing past "Loading..." on first real-browser test.
  Every request from the Vite dev server to the API was silently blocked by the browser's CORS
  policy; no server-side error, no stack trace, just a network failure the type-checker and unit
  tests would never catch.
- How I noticed (the AI): opened the app in an actual Playwright-driven browser instead of stopping
  at `tsc` passing — console/network error listeners on the page caught the CORS rejection directly.
- Fix: added `CORSMiddleware` to `backend/app/main.py`, scoped to the known dev origin.
